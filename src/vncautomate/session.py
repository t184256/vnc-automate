...
from .client import VNCAutomateFactory
...
def default_args(path, language="eng", loglevel="info"):
    assert loglevel in ["critical", "error", "warning", "info", "debug"]
    assert  os.path.exists(path)

    args = Namespace()
    # 3-character ISO 639-2 language code
    # Language is used as a parameter of tesseract and must be available as a language file for it, or it will return:
    #  Tesseract couldn't load any languages!
    setattr(args, 'language', language)
    setattr(args, 'logging', loglevel)
    setattr(args, 'debug_boxes', f"{path}/boxes.png")
    setattr(args, 'debug_screen', f"{path}/screen.png")
    setattr(args, 'debug_gradients_x', f"{path}/x.png")
    setattr(args, 'debug_gradients_y', f"{path}/y.png")
    setattr(args, 'debug_dir', path)
    setattr(args, 'screenshot_dir', path)
    return args
